...
def stroke_data_normalize(casia_filename, dink_filename, norm_width, norm_height):
...
def stroke_data_add_comp(han_filename, comp_filename, dink_norm_filename, dink_comp_filename):
...
def make_sorder_train_data(han_out_list, input_filename, h5_filename, output_filename):
    ...
    for each in f_d2:
        ...
        han = dink_data['han']
        if len(han_out_list) > 0 and han not in han_out_list:
            continue
        if len(han_out_list) > 0:
            if han not in map_out:
                map_out[han] = {'han':[], 'labels':[], 'points':[]}
        else:
            if 'stroke' not in map_out:
                map_out['stroke'] = {'han':[], 'labels':[], 'points':[]}
        ...
        for i in range(int(len(strokes))):
            ...
            for j in range(len(strokes) -  1, len(strokes)):
                ...
                if len(han_out_list) > 0:
                    map_out[han]['han'].append(han)
                    map_out[han]['labels'].append(np.asarray(out_ids, dtype=np.int32))
                    map_out[han]['points'].append(np.asarray(point_items, dtype=np.int32))
                else:
                    map_out['stroke']['han'].append(han)
                    map_out['stroke']['labels'].append(np.asarray(out_ids, dtype=np.int32))
                    map_out['stroke']['points'].append(np.asarray(point_items, dtype=np.int32))
    ...
    for han in map_out:
        if han == 'stroke':
            fullname = h5_filename
        else:
            basename = os.path.basename(h5_filename)
            pathname = os.path.dirname(h5_filename)
            items = basename.split('.')
            if len(items) == 2:
                filename = '{}_{}.{}'.format(items[0], hex(ord(han)), items[1])
            else:
                filename = '{}.{}.h5'.format(items[0], hex(ord(han)))
            fullname = os.path(pathname, filename)

        f_h5 = h5py.File(fullname, 'w')     

        data_han = np.array(map_out[han]['han'])
        dtype_han = h5py.special_dtype(vlen=str)
        ds = f_h5.create_dataset('han', data_han.shape , dtype=dtype_han)
        ds[:] = data_han

        data_label = np.asarray(map_out[han]['labels'], dtype=object)
        dtype_label = h5py.special_dtype(vlen=np.int32)
        try:
            f_h5.create_dataset('labels', data=data_label, dtype=dtype_label)
        except:
            data_label = np.asarray(map_out[han]['labels'], dtype=np.int32)
            f_h5.__delitem__('labels')
            f_h5.create_dataset('labels', data=data_label)

        data_point = np.asarray(map_out[han]['points'], dtype=object)
        dtype_point = h5py.special_dtype(vlen=np.int32)
        f_h5.create_dataset('points', data=data_point, dtype=dtype_point)
        
        f_h5.close()

        hdf = H5HanDataset(fullname)
        print(len(hdf), hdf[0])

def make_stroke_train_data(han_filename, comp_filename, han_out_list, input_filename, h5_filename, output_filename):
    ...
    for each in f_d2:
        ...
        for i in range(1):
            ...
            if len(han_out_list) > 0:
                map_out[han]['han'].append(han)
                map_out[han]['labels'].append(np.asarray(han_stroke_ids, dtype=np.int32))
                map_out[han]['points'].append(np.asarray(point_items, dtype=np.int32))
            else:
                map_out['stroke']['han'].append(han)
                map_out['stroke']['labels'].append(np.asarray(han_stroke_ids, dtype=np.int32))
                map_out['stroke']['points'].append(np.asarray(point_items, dtype=np.int32))
